...
use super::{bounded_limit, bounded_offset, create_scan_session, insert_file_entry, list_children_ids, MAX_LIMIT};
...
fn inserts_and_lists_direct_children_only() {
    ...
    let root_id = insert_file_entry(&connection, session_id, None, "tmp", "C:/tmp", 30, true).unwrap();
    let child_id = insert_file_entry(&connection, session_id, Some(root_id), "a.bin", "C:/tmp/a.bin", 20, false).unwrap();
    let _grandchild_id = insert_file_entry(&connection, session_id, Some(child_id), "nested.bin", "C:/tmp/nested.bin", 10, false).unwrap();
